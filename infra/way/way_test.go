...
func TestReArrange(t *testing.T) {
	...
	for _, input := range inputs {
		r.Handle(input.method, input.path, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
	}

	r.ReArrange()

	for i, path := range expectedOrders {
		out := fmt.Sprintf("/%s", strings.Join(r.routes[i].segs, "/"))
		if out != path {
			t.Errorf("expecting %s instead of %s in current path order", path, out)
		}
	}
}
